...
Set of ETL (Extract->Transform->Load) tools emulate a one-way replication system to keep a child geo-database (GDB) synced 
...
4. Replicated features are stored in a report specific feature class.
...
#### Data Structure Limitations
 ...
 - Features to be replicated must contain a unique ID filed.
...
- If a replicated report features uses feature linked annotation, the font size is based on the scale and the label properties are based on report specific attributes.
...
In order to keep a child GDB synced with a parent GDB, a set of ETL tools (written in python) found [here](./Install/Toolbox/bin) have been created to:
...
Instead of immediately adding features to the report feature class, I've opted to store new features to a temporary feature class and have the user append the new features.

Some built in features with the script is the ability to:
 - Add features to a specific figure.
...
Some built in features with the script is the ability to:
 - Update attributes for a specific figure.
...
the figure. This is an entirely optional feature class that is required.
...
#### Contact
 ...
 Feel free to reach out to me by [email](mailto:sddocherty@gmail.com) for more info about the data model and if you find any bugs, open up an issue in the repository.
